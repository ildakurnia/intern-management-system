...
class AppServiceProvider extends ServiceProvider
{
    ...
    public function boot(): void
    {
        \Illuminate\Support\Facades\View::composer('*', function ($view) {
            if (! auth()->check()) {
                $view->with('menuData', [(object) ['menu' => []]])
                    ->with('topbarNotifications', collect())
                    ->with('topbarUnreadCount', 0);
            ...
            $cacheKey = 'ims.menu.' . $user->id . '.' . md5($roleSignature . '|' . (string) $user->division_id . '|' . (string) optional($user->updated_at)->timestamp);

            $menu = Cache::remember($cacheKey, now()->addMinutes(5), function () use ($user) {
                $menu = [];
            ...
            $notifications = Cache::remember(
                "ims.topbar.notifications.{$user->id}",
                now()->addSeconds(20),
                function () use ($user) {
                    ...
                    return Notification::forUser($user->id)->limit(10)->get();
                }
            );

            $view->with('menuData', [(object) ['menu' => $menu]])
                ->with('topbarNotifications', $notifications)
                ->with('topbarUnreadCount', $notifications->whereNull('read_at')->count());
        });
    }
}
